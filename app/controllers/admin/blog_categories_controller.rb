class Admin::BlogCategoriesController < Admin::AdminController
  ...
  def index
    ...
    @blog_categories = BlogCategory.page(params[:page]).per(params[:per_page]).order('id desc')
  end
  ...
  def create
    ...
    respond_to do |format|
      if @blog_category.save
        format.html { redirect_to admin_blog_categories_path, notice: t(:blog_category, scope: [:activerecord, :models])+t(:message_success_create) }
  ...
  def update
    respond_to do |format|
      if @blog_category.update(blog_category_params)
        format.html { redirect_to admin_blog_category_path(@blog_category), notice: t(:blog_category, scope: [:activerecord, :models])+t(:message_success_create) }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to admin_blog_categories_url, notice:  t(:blog_category, scope: [:activerecord, :models])+t(:message_success_delete) }
